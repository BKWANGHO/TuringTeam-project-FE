...
import AxiosConfig, { instance } from "./components/common/configs/axios-config";
...
export default function Home() {
  ...
  const getLoginMsg = useSelector(getLogin)
  ...
  const handleSubmit = () => {
    ...
    dispatch(login(user))
    console.log('버튼 누름'+getLoginMsg)
  }
  
  useEffect(() => {
    if (getLoginMsg === "SUCCESS") {
      console.log(getLoginMsg)
      router.push(`${PG.BOARD}/list`)
    } else if (getLoginMsg === 'FAILURE') {
      alert('로그인 실패')
    } else {
    }
  }, [getLoginMsg])
